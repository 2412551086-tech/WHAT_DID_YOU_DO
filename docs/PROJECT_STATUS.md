...
- `pnpm run build`
- `pnpm test --runInBand`
- `pnpm run test:e2e`：12/12
- iOS `build-for-testing` 通过
- iOS Simulator XCTest 完整运行通过；Keychain 真机环境相关用例按既有条件跳过
...
3. 隔离开发兑换码与生产配置，正式付费前接入 StoreKit 和服务端收据校验。
