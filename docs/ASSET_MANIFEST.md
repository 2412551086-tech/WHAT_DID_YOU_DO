...
表中的分类只是选择该图标时的推荐值。用户可以在创建表单中独立选择 `烹饪`、`清洁`、`洗护`、`整理`、`照顾`、`家庭事务`；频率提示不生成重复任务。默认值可由用户修改；服务端最终限制时长为 1...180 分钟、倍率为 0.5x...2.0x。免费账号基础保存 2 项自定义家务并叠加成就奖励，开发测试高级账号产品显示不限、服务端保护上限 100；常用页最多同时展示 2 个尚未使用的空白位置。

## 成就徽章

- 正式运行资源位于 `apps/ios/Resources/Assets.xcassets/Achievements`。
- 当前已有 27 枚正式 `.imageset`，用于成长、专长、互动、家庭、搭档、长期和隐藏成就。
- 服务端仍以稳定 achievement key 为唯一标识；iOS 使用 key 到 Asset Catalog 名称的映射，禁止依赖设计原稿文件路径。
- 尚未拥有独立图稿的关联成就暂复用同主题资源，不能在文档中表述为“47 枚独立视觉已完成”。
- 锁定状态由 SwiftUI 统一使用灰度、透明度和锁图层实现，不维护第二套灰色 PNG。
- 成就设计原稿不提交 GitHub；只提交 App 实际运行需要的压缩资源。
