...
- 免费账号每个家庭 2 个基础共享自定义家务槽位并叠加成就奖励，测试高级账号产品显示不限、服务端保护上限 100；常用页仅显示已有自定义项和接下来的 2 个空位。支持 10 项图标、自定义名称、统计大类、默认时长和 0.5x...2.0x 积分倍率。
...
- 手机号登录页不要求昵称；创建/加入家庭流程和个人页支持开发用户昵称修改。
...
- 跟随系统、浅色、深色三种外观模式；DesignSystem 颜色、表面和控件统一自适应。
- 常用家务支持正常滚动、轻触记录、长按进入布局、拖动排序/移除和底部进入家务库；交互状态在弹窗、切换 Tab 或离开页面时复位。
- 高级家庭成员可选择沿用 OWNER 家庭布局或维护个人布局；免费家庭继续只使用 OWNER 共享布局。
- 成就基础体验：Home/Profile 入口、进行中/收藏柜、详情可见性、最近成功缓存、同步/错误状态和同批次聚合解锁反馈；客户端不自行判定或发放奖励。
...
- MEMBER 退出时把成员关系标记为 `LEFT`，不物理删除；再次申请加入复用原成员关系并回到 `PENDING`。
- ChoreRecord 保存创建者昵称、身份和头像快照，历史动态不受后续资料变化或退出/重新加入影响。
...
- 语音识别、常做任务和重复任务。
- 评论、通知、任务排班、积分兑换。
- 家务专长、搭档协作和家庭互动成就；成就阶段 1–4 已完成，后端判定仍由功能开关灰度控制。
- 多管理员和成员退出/移除策略。
