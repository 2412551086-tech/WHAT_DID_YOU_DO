...
首次启动免登录创建本机体验家庭，或先输入邀请码再认证加入 → OWNER 审核 → 选择家务与实际耗时 → 创建积分记录 → 查看本周/最近动态、排行和可切换月份的月报 → 点赞或软删除记录。

开发环境使用 `DEV` 身份联调；正式发行按区域展示 Apple、微信/Google 与邮箱一次性验证码。
...
- Auth Gate 不要求预先配置家庭；创建/加入家庭流程和个人页支持昵称修改。
- OWNER 可编辑家庭名称；所有 ACTIVE 成员可查看本人或其他成员滚动近 30 天的未删除家务动态。
...
- 成就基础体验：Home/Profile 入口、统一成就网格、详情、最近成功缓存、同步/错误状态和同批次聚合解锁反馈；客户端不自行判定或发放奖励。
- 成就中心统一展示已解锁与未解锁徽章；已解锁优先，带奖励的未解锁项排在其余未解锁项之前。Home 与成就中心提供最多 3 项可横滑“即将完成”目标，中央解锁弹层支持同批次多项左右滑动查看。
- 成就是否向家庭成员展示使用一个成员级总开关；“我的成就”入口不显示已完成/总数量。
- 已接入 27 枚正式成就图片；尚未独立绘制的规则复用最接近的主题素材并保留锁定态处理。
- 首页家庭总积分卡可进入家庭周报二级页；个人统计卡可进入个人周报二级页。二级页复用当前所选周的数据，不新增后端接口。
- 家庭动态按家庭时区显示明确的“周几 HH:mm”；人物卡背景跟随当前形象主色。
- 家庭成员列表中的本人和其他 ACTIVE 成员均可进入同一近 30 天动态详情。
- 家务记录创建者可在动态中左滑编辑实际耗时和高级倍率；编辑后刷新积分、排行、月报并触发异步成就重新评估。
...
- 家庭级自定义家务创建、编辑、查询和归档；免费版基础 2 个并叠加成就奖励，高级版产品显示不限且服务端内部保护 100 个。免费版仅 OWNER 管理，高级 ACTIVE 成员可管理。
...
- ChoreRecord 保存创建者昵称、身份和头像快照。历史昵称/身份使用记录快照；ACTIVE 成员头像优先读取当前家庭成员 `avatarKey`，换形象后既有动态同步显示新头像，成员退出时回退到快照。
...
- 语音识别、常做任务和重复任务。
- 评论、通知、任务排班、积分兑换。
- 成就正式生产灰度、完整独立徽章资源、外部告警与通知系统；本地阶段 1–7 和 iOS 基础体验已经完成，后端判定仍由功能开关/家庭 allowlist 控制。
- 多管理员和成员退出/移除策略。
