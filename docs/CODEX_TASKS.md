...
- [x] 免费基础 2 个并叠加成就奖励 / 高级内部保护 100 个共享自定义家务槽位和 10 项图标库；常用页仅展示接下来的 2 个空位。
...
- [x] MEMBER 退出保留历史关系、重新加入复用成员身份；历史昵称/身份保留记录快照，历史头像优先展示当前家庭成员形象。
- [x] 本周家庭总分与个人统计二级分析页，包含趋势、成员贡献、类别和本周记录。
- [x] 家庭动态时间改为家庭时区内明确的“周几 HH:mm”。
- [x] Profile 身份卡跟随当前形象主色，成员列表包含本人且统一进入近 30 天动态详情。
- [x] 记录创建者可左滑编辑实际耗时和高级倍率，保存后重新聚合统计并触发成就同步。
...
- [x] 将 Premium 自定义家务升级为产品“不限”口径，并实现每家庭 100 个未归档项目的服务端保护上限。
- [x] 新增成就定义、事件、进度、解锁批次、成员/家庭/搭档成就、资格快照、家庭奖励和审计数据模型。
- [x] 新增服务端 `occurredAt`、10 秒记录撤销接口及对应 e2e；不提供回收站。
- [x] seed 幂等写入 59 条 V1 成就定义，`ACHIEVEMENTS_ENABLED` 默认关闭，旧记录时间安全回填。
- [x] 实现数据库 Outbox、请求幂等键、异步 worker、处理租约、指数退避、死信查看和人工重放，保证家务成功不依赖成就处理结果。
- [x] 实现按家庭时区快照计算活跃日、严格连续和滚动 25/30，并保证奖励发放幂等。
- [x] 实现首记、累计活跃 3/5/7 日、严格连续 7/14 日和 25/30 习惯成就及后端边界测试。
- [x] 新增成就 summary/me/detail/visibility 查询接口。
- [x] 新增 iOS `AchievementsView`，入口放在 Home/Profile，不增加第五个 Tab。
- [x] 接入本地缓存、同步状态、统一成就网格、详情页、家庭可见总开关和同批次聚合解锁反馈。
- [x] Home 与成就页展示最多 3 个即将完成成就；多项解锁使用页面中央横向滑动反馈。
- [x] 实现次数/时长/类别覆盖专长、主题资格过滤、铜银金跨级聚合与每日防刷上限。
- [x] 实现 3 项个人回应、6 项家庭协作和首个做饭/洗碗搭档成就，包含关系日上限、资格快照、参与人叙事和单人家庭显隐。
- [x] 实现家庭长期里程碑、5 个隐藏彩蛋、退出/重入归档、账号匿名化、成就档案、DIRTY 对账修复、健康检查和家庭灰度开关。
- [ ] 在预发布环境接入外部告警，执行数据库备份/恢复、事件重放和全量对账演练。
- [x] 首批 27 枚正式成就资源已接入 Asset Catalog，并提供关联成就复用映射、锁定态和深色模式处理。
- [ ] 将复用映射逐步替换为完整 47 枚独立成就视觉，并完成最终视觉微调。
- [ ] 为本周二级分析、家庭时区动态时间和记录编辑补充稳定截图/交互回归。
- [ ] 使用 `ACHIEVEMENT_FAMILY_ALLOWLIST` 完成 TestFlight 小家庭 1/7/30 日灰度观察。

完整规则见 `docs/家庭家务成就系统完整开发文档.md`，工程阶段见 `docs/ACHIEVEMENT_DEVELOPMENT_PLAN.md`。当前可标记为“本地工程完成”，但在 TestFlight 灰度、发布环境演练和独立视觉补齐前不要写成“生产上线”。
