...
struct FamilyDashboardView: View {
    ...
    private var monthPill: some View {
        HStack(spacing: 2) {
            ...
            .buttonStyle(.plain)
            .disabled(!viewModel.canSelectNextReportMonth || viewModel.isLoading)
    ...
    private var leaderIllustration: some View {
        Image(viewModel.monthlyLeaderIllustrationAsset)
            .resizable()
            .scaledToFit()
            .frame(width: dynamicTypeSize.isAccessibilitySize ? 190 : 166, height: 202)
    ...
    private func rankingRow(member: FamilyMember, index: Int) -> some View {
        ...
        return HStack(spacing: 10) {
            ...
            Text("\(member.monthlyPoints) 分")
                .font(.system(size: 16, weight: isCurrentUser ? .semibold : .medium, design: .default))
    ...
    private var distributionSection: some View {
        DSFloatingSurface(cornerRadius: 20, padding: 16, elevation: .secondary) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if themeRows.isEmpty && categoryRows.isEmpty {
                    Label(categoryEmptyMessage, systemImage: "chart.pie")
                        .font(DSFont.functionalBody)
                        .foregroundStyle(DSColor.mutedInk)
    ...
    private var reportHeadline: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            Text(viewModel.monthlyReport?.headline ?? "本月战局待开启，第一笔功劳等你登场。")
                .font(.system(size: 14, weight: .regular, design: .default))
                .foregroundStyle(DSColor.ink)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var leaderPoints: Int {
        leadingMember?.monthlyPoints ?? 0
    }

    private var leaderCompletionText: String {
        guard let leadingMember else { return "等第一笔家务登场" }
