...
14. OWNER 可编辑家庭名称；ACTIVE 成员可查看本人或其他成员滚动近 30 天未删除动态，家主转让从目标成员详情发起。
...
19. MEMBER 退出家庭只标记 `LEFT` 并保留 FamilyMember；重新申请复用原关系，不按昵称匹配身份。
20. 历史 activity 的昵称和身份使用记录快照；成员仍在当前家庭时头像跟随其最新 avatarKey，找不到当前成员关系时回退到记录头像快照。
21. 本周家庭总积分和个人统计均可进入二级分析页，复用当前周动态与周排行榜计算趋势、贡献和类别统计。
22. 记录创建者可左滑编辑实际耗时；家庭高级版还可编辑本次积分倍率，保存后刷新统计并重新触发成就同步。

DEV 登录只用于本地联调，不是生产认证。Apple、微信、Google 和邮箱验证码的真实接口尚未接入。
...
- Profile 顶部依次显示家庭名称、身份、昵称；OWNER 可编辑家庭名称，所有 ACTIVE 成员可进入本人或其他成员近 30 天详情。
- Profile 支持修改本人昵称，并通过“账户与安全”二级页管理退出家庭、退出登录和永久注销；OWNER 普通退出前必须先完成家主转让，注销时由后端自动转让或删除单人成立的家庭。成员列表包含本人，并统一进入成员动态详情。
- Profile 顶部身份卡跟随当前人物形象的主色；“我的成就”入口不展示完成数徽标。
- Profile 提供跟随系统、浅色和深色三种外观选择。
- 成就系统阶段 1–7 的本地工程已完成：59 条定义、异步 Outbox/worker、成长/专长/互动/家庭/搭档/长期/隐藏成就、统一成就网格、详情页、家庭可见总开关和中央横向解锁反馈均已接入。
- Home 与成就页显示最多 3 个即将完成成就；当前有 27 枚正式 Asset Catalog 成就资源，其余关联成就暂复用同主题资源，完整 47 枚独立视觉仍待补齐。
...
- Apple、邮箱验证码及对应发行区域的微信或 Google 认证。
- 真实头像和图片凭证上传。
